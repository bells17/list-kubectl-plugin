...
func NewListPrintFlags() *PrintFlags {
	outputFormat := ""
	noHeaders := false
...
// ListOptions provides information required to update
...
// NewListOptions returns a ListOptions with default chunk size 500.
func NewListOptions(streams genericclioptions.IOStreams) *ListOptions {
	return &ListOptions{
		PrintFlags:  NewListPrintFlags(),
...
// NewCmdList provides a cobra command wrapping ListOptions
func NewCmdList(streams genericclioptions.IOStreams) *cobra.Command {
	o := NewListOptions(streams)

	cmd := &cobra.Command{
		Use:                   "list [(-o|--output=)json|yaml|wide|custom-columns=...|custom-columns-file=...|go-template=...|go-template-file=...|jsonpath=...|jsonpath-file=...] (TYPE[.VERSION][.GROUP] [NAME | -l label] | TYPE[.VERSION][.GROUP]/NAME ...) [flags]",
...
func (o *ListOptions) watch(f cmdutil.Factory, cmd *cobra.Command, args []string) error {
	r := f.NewBuilder().
		Unstructured().
		NamespaceParam(o.Namespace).DefaultNamespace().AllNamespaces(o.AllNamespaces).
		FilenameParam(o.ExplicitNamespace, &o.FilenameOptions).
...
func addOpenAPIPrintColumnFlags(cmd *cobra.Command, opt *ListOptions) {
...
func addServerPrintColumnFlags(cmd *cobra.Command, opt *ListOptions) {
